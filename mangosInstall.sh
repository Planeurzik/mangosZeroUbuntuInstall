...
mangospath=$(pwd)
clientpath=$mangospath/client
nproc=$(nproc)
mangospassword=mangosadmin
ip=$(curl https://ipinfo.io/ip)
wget https://dev.mysql.com/get/mysql-apt-config_0.8.15-1_all.deb && sudo dpkg -i $mangospath/mysql-apt-config_0.8.15-1_all.deb
rm $mangospath/mysql-apt-config_0.8.15-1_all.deb
sudo apt-get update -y && sudo apt-get full-upgrade -y
sudo mysql_secure_installation
sudo mysql -e "GRANT ALL ON *.* to 'mangos'@'%' IDENTIFIED BY '$mangospassword';"
...
cd server && cd build && cmake .. -DCMAKE_INSTALL_PREFIX=$mangospath/server && make -j$nproc && make install
...
#        Example: "/home/planeur/wow/server/mangos/share/mangos"
#
#    LogsDir
#        Logs directory setting.
#        Important: Logs dir must exists, or all logs need to be disabled
#        Default: "" - no log directory prefix. if used log names arent absolute paths
...
#                X = LoginDatabaseConnections + WorldDatabaseConnections + CharacterDatabaseConnections + 1
...
#        DO NOT CHANGE THIS UNLESS YOU _REALLY_ KNOW WHAT YOURE DOING
#
################################################################################
...
DataDir                      = "../etc"
LogsDir                      = ""
LoginDatabaseInfo            = "127.0.0.1;3306;mangos;'$mangospassword';realmd"
WorldDatabaseInfo            = "127.0.0.1;3306;mangos;'$mangospassword';mangos0"
CharacterDatabaseInfo        = "127.0.0.1;3306;mangos;'$mangospassword';character0"
...
#                -1 (for Mods, GMs and Admins only)
#                -2 (for GMs and Admins only)
#                -3 (for Admins only)
#
...
#        Default: "" (dont load all grids at startup)
...
#                 0 - Dont log anything
#        Default: 1 - Only log GM tickets responses and the following conversation until closing the ticket
#                 2 - Log all whispers
#
#    WorldLogFile
...
#        Default: 0 - dont include dumping chars to log
...
#
#    SkipCinematics
#        Disable in-game script movie at first characters login(allows to prevent buggy intro in case of custom start location coordinates)
...
#
#    MountCost
#        The cost in copper of the regular mounts usually purchased at level 40.
#        Default: 100000 (10g)
#
#    TrainMountCost
#        The cost in copper for training the apprentice riding skill for regular mounts usually purchased at level 40.
#        Default: 900000 (90g)
#
#    MinTrainMountLevel
#        The minimal level at which players can train the apprentice riding skill for regular mounts.
#        Default: 40 (level 40)
#
#    EpicMountCost
#        The cost in copper for epic mounts usually purchased at level 60.
#        Default: 1000000 (100g)
#
#    TrainEpicMountCost
#        The cost in copper for training the journeyman riding skill for epic mounts usually purchased at level 60.
#        Default: 9000000 (900g)
#
...
#
#    AllFlightPaths
#        Players will start with all flight paths (Note: ALL flight paths, not only players team)
#        Default: 0 (true)
#                 1 (false)
#
#    InstantFlightPaths
...
#                 3 (check if color, entry and name dont contradict each other. For correct work, please assure
...
#        Objects visibility update called, when distance between current objects position and position,
...
#        0 = Disabled (default)
#        1 = Enabled
#    Rate.Mining.Autopooling
#        Percentage of spawned mining nodes per zone
#
#    Rate.Talent
#        Talent Point rates
#        Default: 1
...
#         Modifies the speed of players ghosts, removed upon reviving, not permanent/saved, in non-BG and BG maps
...
#        Default: 0 (normal - invite as much players to bg as possible, dont bother with ballance)
#                 1 (Experimental - dont allow to invite much more players of one faction)
#
...
#
#    Ra.MinLevel
#        Minimum level thats required to login,3 by default
...
#        Default: 30
#                 0  - Dont delete any characters, they stay in the database forever.
#
################################################################################
...
#                     0 - (Disabled, client wont be kicked)
#
...
'>$mangospath/server/etc/mangosd.conf
echo '################################################################################
